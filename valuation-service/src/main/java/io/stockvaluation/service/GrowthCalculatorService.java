...
public class GrowthCalculatorService {
    ...
    public static GrowthDto calculateGrowth(List<Double> historicalRevenues , List<Double> historicalMargins, double marginMin, double marginMax) {
        if (historicalRevenues == null || historicalMargins == null) {
            throw new InsufficientFinancialDataException("Historical revenue/margin data is missing.");
        }
        if (historicalRevenues.size() < 3 || historicalMargins.size() < 3) {
            throw new InsufficientFinancialDataException("insufficient dimension for growth calculation; need at least 3 periods.");
        }

        List<Double> revenueGrowthRates = new ArrayList<>();
        for(int i=1; i < historicalRevenues.size(); i++) {
            if (historicalRevenues.get(i - 1) == 0.0) {
                throw new InsufficientFinancialDataException("Historical revenue contains zero value; cannot derive growth rates.");
            }
        ...
        if (revenueGrowthRates.size() < 2 || marginChanges.size() < 2) {
            throw new InsufficientFinancialDataException("insufficient dimension for growth correlation; need at least 2 growth deltas.");
        }
