...
import static io.stockvaluation.service.SpecialCompanies.reAdjustROIC;
...
public class ValuationOutputService {
    ...
    // private static Double DEFAULT_MATURE_MARKET = 4.27; // in %
    private static Double DEFAULT_MATURE_MARKET = 3.94; // in % sep 01 2025
    ...
    private Double[] calculateReinvestment(FinancialDataInput valuationInputDTO, Double[] revenues,
        ...
        terminalROIC = reAdjustROIC(valuationInputDTO, terminalROIC);
    ...
    private Double[] calculateCostOfCapital(FinancialDataInput financialDataInput, int arrayLength) {
        ...
        // Calculate terminal cost: riskFreeRate (4.6%) + DEFAULT_MATURE_MARKET (3.94%)
        // = 8.54%
        Double terminalCostOfCapital = riskFreeRate + DEFAULT_MATURE_MARKET;
    ...
    private Double[] calculateROIC(Double[] investedCapital, Double[] ebit1MinusTax,
        ...
        assumptionsValue = reAdjustROIC(financialDataInput, assumptionsValue);
    ...
    private Double[] calculateAveragedSalesToCapitalFromSectors(FinancialDataInput financialDataInput,
        ...
        for (SegmentResposeDTO.Segment segment : financialDataInput.getSegments().getSegments()) {
    ...
    public ValuationOutputDTO getValuationOutput(String ticker, final FinancialDataInput valuationInputDTO,
        ...
        // all these convertors in advanced and pass the results in
        ...
        return this.addStory(commonService.getStory(valuationOutputDTOWithStory, valuationInputDTO, ticker, addStory));
    }
    ...
    public ValuationOutputDTO addStory(ValuationOutputDTO valuationOutputDTO) {
        String growthStoryTemplate = "Over the next few years, revenue is expected to grow by %s%%, %s than industry average of %s%%.";
        String profitabilityTemplate = "In the growth phase company makes %s%% profit on every dollar of sales, %s than industry average of %s%%.";
        String riskTemplate = "Investors expect at least %s%% return to justify the risk of holding stock.";
        String competitiveAdvantages = "Market expect revenue to grow by %s%%, and make %s%% profit on every dollar of sales for current price %s to justify. Are you comfortable with this expectation?";

        if (valuationOutputDTO.getCalibrationResultDTO() != null
                && valuationOutputDTO.getCurrency().equalsIgnoreCase(valuationOutputDTO.getStockCurrency())) {
            valuationOutputDTO.setCompetitiveAdvantages(
                    String.format(
                            competitiveAdvantages,
                            df.format(valuationOutputDTO.getCalibrationResultDTO().getRevenueGrowth()),
                            df.format(valuationOutputDTO.getCalibrationResultDTO().getOperatingMargin()),
                            valuationOutputDTO.getCompanyDTO().getPrice() + " " + valuationOutputDTO.getCurrency()));

        }

        double revenueGrowthRateIndustry = valuationOutputDTO.getBaseYearComparison().getRevenueGrowthIndustry();
        Optional<Double> revenueGrowthRateGrowth = Arrays
                .stream(valuationOutputDTO.getFinancialDTO().getRevenueGrowthRate()).skip(3)
                .findFirst();

        if (revenueGrowthRateGrowth.isPresent()) {
            if (revenueGrowthRateGrowth.get() > revenueGrowthRateIndustry) {
                valuationOutputDTO.setGrowthStory(
                        String.format(growthStoryTemplate, df.format(revenueGrowthRateGrowth.get()), "higher",
                                df.format(revenueGrowthRateIndustry)));
            } else {
                valuationOutputDTO.setGrowthStory(
                        String.format(growthStoryTemplate, df.format(revenueGrowthRateGrowth.get()), "lower",
                                df.format(revenueGrowthRateIndustry)));
            }
        }

        double operatingMarginIndustry = valuationOutputDTO.getBaseYearComparison().getOperatingMarginIndustry();
        Optional<Double> operatingMarginGrowth = Arrays
                .stream(valuationOutputDTO.getFinancialDTO().getEbitOperatingMargin()).skip(3)
                .findFirst();

        if (operatingMarginGrowth.isPresent()) {
            if (operatingMarginGrowth.get() > operatingMarginIndustry) {
                valuationOutputDTO.setProfitabilityStory(
                        String.format(profitabilityTemplate, df.format(operatingMarginGrowth.get()), "higher",
                                df.format(operatingMarginIndustry)));
            } else {
                valuationOutputDTO.setProfitabilityStory(
                        String.format(profitabilityTemplate, df.format(operatingMarginGrowth.get()), "lower",
                                df.format(operatingMarginIndustry)));
            }
        }

        Optional<Double> costOfCapital = Arrays.stream(valuationOutputDTO.getFinancialDTO().getCostOfCapital()).skip(1)
                .findFirst();

        costOfCapital.ifPresent(aDouble -> valuationOutputDTO.setRiskStory(String.format(
                riskTemplate, df.format(aDouble))));
    ...
    private void calculateSectorFinancialMetrics(FinancialDTO financialDTO, FinancialDataInput financialDataInput,
        ...
        for (SegmentResposeDTO.Segment segment : financialDataInput.getSegments().getSegments()) {
            ...
            terminalRoicValue = reAdjustROIC(financialDataInput, terminalRoicValue);
