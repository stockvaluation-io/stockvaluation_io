...
public class OptionValueService {
    ...
    public OptionValueResultDTO calculateOptionValue(String ticker, Double strikePrice, Double avgMaturity, Double optionStanding, Double standardDeviation) {
        ...
        double optionExpire = avgMaturity;
        double yieldStock = 0.00; // constant
        double riskFreeRate = companyDataDTO.getCompanyDriveDataDTO().getRiskFreeRate(); // in %
        double numberOfsharesOutstanding = companyDataDTO.getFinancialDataDTO().getNoOfShareOutstanding();
        ...
        log.info("d1 res is = {}", df.format(d1Res));
        log.info("nd1 res is = {}", df.format(nd1));
        log.info("d2 res is = {}", df.format(d2));
        log.info("nd2 res is = {}", df.format(nd2));

        double valuePerOption = calculateValuePerOption(currentStockPrice, strikePrice, avgMaturity, nd1, nd2, riskFreeRate / 100, yieldStock / 100);
...
//      formula =   d1 - standardDeviation * Math.sqrt(avgMaturity);
    ...
    public double calculateNd1(double d1) {
        // Create a normal distribution object (mean = 0, standard deviation = 1)
        NormalDistribution normalDist = new NormalDistribution(0, 1);
        double result = normalDist.cumulativeProbability(d1);
    ...
    public static double calculateNd2(double d2) {
        NormalDistribution normalDist = new NormalDistribution(0, 1);
        double result = normalDist.cumulativeProbability(d2);
        log.info("Nd2 : {} ", result);
        return result;
    }
